class Autocomplete {
    ...
    getMatches( text ) {
      ...
      */
      let selectElement =  document.querySelector ('.autocomplete__input')
      let dict={
          'text':'',
          'value':'',
      }
      ...
      for (let i=0; i < selectElement.options.length; i++){
          if (selectElement.options[i].text.includes(this.searchInput.value)){
              dict.text = selectElement.options[i].text
              dict.value = selectElement.options[i].value
              list.push({'text':dict.text,'value':dict.value})
          }
      }
      return list
    }
